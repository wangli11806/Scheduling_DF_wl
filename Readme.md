...
| name | TEXT UNIQUE | A班/B班/C班/T班/E班/F班/休息/放休/请假 |
| info | TEXT | 早班/常规班/晚班 等 |
...
POST   /api/schedules/import             Excel 导入（列表格式）
POST   /api/schedules/import-matrix      Excel 导入（矩阵格式）
GET    /api/schedules/export             Excel 导出
```

矩阵模板：第一列员工姓名，第一行日期（支持 YYYY-MM-DD、M/D、D日等格式），单元格填班次代码（A=A班, B=B班, C=C班, T=T班, E=E班, F=F班, 休=休息, 假=请假, 空=休息）。
...
- 班次颜色方案：A班(蓝)、B班(青)、C班(紫)、T班(靛)、E班(黄)、F班(粉)、休息/放休(灰)、请假(橙)
