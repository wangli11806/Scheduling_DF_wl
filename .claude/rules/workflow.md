...
6. 确认需求无误后，才开始改代码
7. **代码改完后，本地启动 Flask（端口 5001），告诉用户访问 http://127.0.0.1:5001 验收**
8. **用户确认验收通过后，才 git commit + push + 部署到 ECS**

## 部署前置条件

**绝对禁止在用户验收前直接部署到 ECS。** 每次代码改动后必须：

1. 本地启动 Flask 验证：`python -c "from app import app; app.run(port=5001)"`
2. 浏览器访问 `http://127.0.0.1:5001` 自测功能
3. 告诉用户："已在本地启动，请访问 http://127.0.0.1:5001 验收"
4. 用户确认没问题后，才提交代码并部署到 ECS

**Why:** 曾多次在用户未验收的情况下直接部署到生产环境，导致有问题的代码上线。本地验收可以在部署前发现大部分问题。
